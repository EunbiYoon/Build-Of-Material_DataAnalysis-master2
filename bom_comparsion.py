...
###########################GERP###############################
gerp=pd.read_excel('F:/NPTGERP/1-MaterialCost/TestFile/Test5/gerp.xlsx')
# #파일에 따라 추가하기
...
gerp.to_excel('F:/NPTGERP/1-MaterialCost/TestFile/Test5/gerpresult.xlsx')

###########################NPT###############################
npt=pd.read_excel('F:/NPTGERP/1-MaterialCost/TestFile/Test5/npt.xlsx')
...
npt.to_excel('F:/NPTGERP/1-MaterialCost/TestFile/Test5/nptresult.xlsx')
...
#매칭 안되고 missing 된것
remain_gerp.to_excel('F:/NPTGERP/1-MaterialCost/TestFile/Test5/remaingerp.xlsx')
...
#save file
match_list.to_excel('F:/NPTGERP/1-MaterialCost/TestFile/Test5/matchlist.xlsx')
...
#save file
sub_match_list.to_excel('F:/NPTGERP/1-MaterialCost/TestFile/Test5/submatchlist.xlsx')
...
#save file
match_join.to_excel('F:/NPTGERP/1-MaterialCost/TestFile/Test5/matchjoin.xlsx')
...
#MTL column
mtl=pd.read_excel('F:/NPTGERP/1-MaterialCost/TestFile/Test5/gerp.xlsx',sheet_name='MTL Cost')
...
for i in range(len(final_table)):
    ...
    if part_des.__contains__('Tub,Drum(') and part_match.__contains__('Substitute'):
        for j in range(len(final_table)):
            another_parent=final_table.at[j,"Parent Item"][:-2]
            another_qty=final_table.at[j,"Qty Per Assembly"]
...
#substitute~~ ->Substitute (substitute first change **)
for i in range(len(final_table)):
...
for i in range(len(final_table)):
    final_match=str(final_table.at[i,"match"])
    if final_match.__contains__('price'):
        final_table.at[i,"match"]="Price Change"

#parent~~~ -> True
for i in range(len(final_table)):
    final_match=str(final_table.at[i,"match"])
    final_price=final_table.at[i,"price match"]
    if final_match=="False parent" and final_price==0:
        final_table.at[i,"match"]=True
...
#false column name change
final_table["match"]=final_table["match"].replace("False",False)

# #price match column round 3 -> true condition
...
final_table.to_excel('F:/NPTGERP/1-MaterialCost/TestFile/Test5/final_table.xlsx')
